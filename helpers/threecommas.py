...
def set_threecommas_bot_pairs_macs(logger, api, thebot, newpairs, maxpairs):
    """Update bot with new pairs."""
    #print("<<<< Bot on? ", thebot["is_enabled"])

    logger.debug("Current pairs: %s\nNew pairs: %s" % (thebot["pairs"], newpairs))

    # If number of new pairs is < current max active deals, then reduce max active deals
    new_maxactivedeals = int(0)
    if len(newpairs) < maxpairs:
        new_maxactivedeals = len(newpairs)
        logger.info("Reduced number of max active deals to %s" % (len(newpairs)))
    else:
        new_maxactivedeals = maxpairs
        logger.info("Number of max active deals as maxpairs equal to %s" % (maxpairs))

    if new_maxactivedeals == 0:
    # this is the case in which no pair fits the restrictions imposed, since you can't
    # set max_active_deals to 0, in this case I switch off the bot (if it wasn't already off)
        if not thebot['is_enabled']:
            logger.debug("No new suitable pair: keep the bot off!")
            logger.info("No new suitable pair: keep the bot off!")
            return
        else:
            error, data = api.request(
                entity="bots",
            ...
            logger.debug("No new suitable pair: shut off the bot!")
            logger.info("No new suitable pair: shut off the bot!")
            return
    ...
    if newpairs == thebot["pairs"]:
        ...
        if not thebot['is_enabled']:
            ...
            logger.debug("Re-start the bot (as it was off)!")
            logger.info("Re-start the bot (as it was off)!")
        return
    ...
    error, data = api.request(
        ...
        payload={
            ...
            "martingale_step_coefficient": float(thebot["martingale_step_coefficient"]),
            "max_safety_orders": int(thebot["max_safety_orders"]),
            "max_active_deals": new_maxactivedeals,
            "active_safety_orders_count": int(thebot["active_safety_orders_count"]),
            "safety_order_step_percentage": float(
                thebot["safety_order_step_percentage"]
            ),
