...
* [3Commas Cyber Bot Helpers ](#3commas-cyber-bot-helpers-)
   ...
   * [GalaxyScore bot helper named galaxyscore.py](#galaxyscore-bot-helper-named-galaxyscorepy)
    ...
    * [2nd GalaxyScore bot helper named galaxyscore_macs.py](#galaxyscore-bot-helper-named-galaxyscore_macs.py)
         * [What does it do?](#what-does-it-do)
         * [How does it work?](#how-does-it-work)
         * [Configuration](#configuration)
         * [Example output](#example-output)
...
## 2nd GalaxyScore bot helper named `galaxyscore_macs.py`
By @massimocarpita
Type = trading pair

### What does it do?

It will monitor LunarCrush's GalaxyScores and use the Top X with galaxyscore over a given threshold to create pairs for your 3Comma's composite DCA bots to use.

### How does it work?

- THIS HELPER DOESN'T WORK WELL WITH MULTIPLE DEALS ON SAME PAIR! DISABLE THIS FEATURE IN THE BOT YOU WANT TO RUN WITH THIS HELPER

The GalaxyScore Top coins with galaxyscore over a given threshold from LunarCrush are downloaded, the base pair of each of the specified 3Comma's bots is determined, from this new pairs are constructed, these are checked against your Blacklist on 3Comma's and the market data on 3Comma's (reflecting Binance or FTX data depending on your exchange) to see if the pairs are valid.
...
NOTE2: SUPER-IMPORTANT!!!!!!! Take care that this helper is a little bit different from the original one. Since it might be that sometimes you have just a few suitable pair available, especially when you set the galaxyscore threshold large enough, the helper will reduce the number of max active deals in the DCA bot to match the number of suitable pairs active (on the other hand, when the number of suitable pairs overcomes the max number of pairs you want in the bot - specified in configurations - the helper will limit the updated pairs to that number). This couldn't be done in the original helper.
Since the helper can't reduce the number of max active deals to 0, when no suitable pairs is available the bot will be shut off! This "mimics" having no pair (no deal will start). When new pairs become available, nonetheless, the helper will recognize this, set the bot on again and update it with the new pairs!
SO: VERY IMPORTANT -> THE HELPER IS ABLE TO RE-START THE BOT IN CASE IT IS OFF WHEN NEW PAIRS ARE AVAILABLE (according to restrictions!). PLEASE BE AWARE THEN THAT IN ORDER TO STOP THE BOT YOU SHOULDN'T JUST TURN IT OFF ON 3COMMAS BUT YOU CAN EITHER a) DELETE IT, b) TURN IT OFF + STOP THIS HELPER, c) SET MAXPAIRS = 0 ON THIS HELPER'S CONFIG FILE.
...
-   **logrotate** - number of days to keep logs. (default = 7)
-   **botids** - a list of bot id's to manage separated with commas
-   **maxpairs** - max number of pairs you want to use in the bot. Set it to 0 if you want to keep the bot off. (default is 10)
-   **maxaltrankscore** - set this lower for galaxyscore bot helper so altrank score of coin gets evaluated too. (default is 1500)
-   **gs_threshold** - threshold for galaxyscore value below which pairs are not considered. Set it to 0 if you don't want to apply the threshold. (default is 0).
-   **3c-apikey** - your 3Commas API key value.
-   **3c-apisecret** - your 3Commas API key secret value.
-   **lc-apikey** - your LunarCrush API key value.
-   **notifications** - set to true to enable notifications. (default = False)
