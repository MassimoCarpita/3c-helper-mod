...
from helpers.threecommas import (
    ...
    load_blacklist,
    set_threecommas_bot_pairs_macs,
)
...
def lunarcrush_pairs(thebot):
    ...
    for entry in lunarcrush:
        try:
            coin = entry["s"]
            galaxyscore = entry["gs"] # get the galaxyscore of the coin
            #print("Pair: " + coin + " - GS: " + str(galaxyscore))
            ...
            if galaxyscore < gs_threshold:
                logger.debug(
                    "Quote currency '%s' does not have high enough galaxyscore (>= %s), skipping and stopping the fetch"
                    % (coin, str(volbtc))
                )
                #print("Stop because we reached gs_threshold")
                break # STOP the fetch for pairs after you get a coin with galaxy score below threshold
                      # from my understanding lunarcrush pairs are fetched sorted by galaxyscore here
            # Populate lists
            populate_pair_lists(
            ...
            if len(newpairs) == maxpairs:
                #print("Stop because we reached maxpairs")
                break # STOP the fetch if I've got already the max # of pairs wanted
    ...
    set_threecommas_bot_pairs_macs(logger, api, thebot, newpairs, maxpairs)
...
while True:
    ...
    maxpairs = int(config.get("settings", "maxpairs"))
    maxacrscore = int(config.get("settings", "maxaltrankscore", fallback=100))
    gs_threshold = int(config.get("settings", "gs_threshold", fallback=0))
